  ...
  const autoSaveMutation = useMutation({
    ...
    onSuccess: (savedPost) => {
      if (!isEditing && savedPost?.id) {
        // Pre-seed cache so the editor loads instantly on redirect — no loading flicker
        queryClient.setQueryData(['/api/admin/blog-posts', String(savedPost.id)], savedPost);
        setLocation(`/edit-post/${savedPost.id}`);
      } else if (isEditing && savedPost) {
        // Update cache directly from save response — avoids a background refetch that
        // can cold-start Vercel and cause the editor to flicker or show an error state
        queryClient.setQueryData(['/api/admin/blog-posts', id], savedPost);
      }
    },
    onError: (error) => {
